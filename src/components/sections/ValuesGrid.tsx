  ...
  return (
    <section className="bg-charcoal px-6 py-24 md:px-10 md:py-32">
      <div className="mx-auto mb-20 grid max-w-[1400px] gap-10 md:grid-cols-2 md:items-end">
        ...
        <Reveal delay={1}>
          <p className="max-w-[38ch] text-[0.9rem] leading-relaxed text-cream/50 md:justify-self-end md:text-right">
            {subtitle}
          </p>
        </Reveal>
      </div>

      <div className="mx-auto grid max-w-[1400px] gap-px bg-cream/10 sm:grid-cols-2 lg:grid-cols-4">
        {items.map((item, index) => (
          <Reveal
            key={item.title}
            delay={delays[index % delays.length]}
            className="card-hover-dark group bg-charcoal p-10"
          >
            ...
            <h3 className="font-display mb-3 text-lg italic leading-snug text-cream">{item.title}</h3>
            <p className="text-[0.82rem] leading-relaxed text-cream/45">{item.description}</p>
          </Reveal>
        ))}
      </div>
    </section>
